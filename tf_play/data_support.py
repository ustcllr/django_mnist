...
def get_training_image_ary():
    ...
    file_name = os.path.join('../dataset', DATASET_NAME, 'train-images.idx3-ubyte')
...
def get_training_label_ary():
    ...
    # 通过数据集二进制文件创建一个二维数组，数据类型为无符号8位
    file_name = os.path.join('../dataset', DATASET_NAME, 'train-labels.idx1-ubyte')
    loaded = np.fromfile(file=file_name, dtype=np.uint8)
    y_ori = loaded[8: ]
    # 先初始化矩阵
    y = np.zeros([60000, 10])
...
def get_recognize_image_ary():
    ...
    # 通过数据集二进制文件创建一个二维数组，数据类型为无符号8位
    file_name = os.path.join('../dataset', DATASET_NAME, 't10k-images.idx3-ubyte')
    loaded = np.fromfile(file=file_name, dtype=np.uint8)
    # 将数组变换为我们需要的a_0
...
def get_recognize_label_ary():
    ...
    # 通过数据集二进制文件创建一个二维数组，数据类型为无符号8位
    file_name = os.path.join('../dataset', DATASET_NAME, 't10k-labels.idx1-ubyte')
    loaded = np.fromfile(file=file_name, dtype=np.uint8)
    y_ori = loaded[8: ]
    # 先初始化矩阵
    y = np.zeros([10000, 10])
